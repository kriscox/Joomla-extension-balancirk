 ...
 *  - An explicit lessonid is used as-is; null falls back to getState('lesson.id').
 *  - The method returns exactly whatever loadObjectList() returns.
 *
 * Mixing up subscribed = 1 (waiting list) vs subscribed = 0 (enrolled) would
 * silently show the wrong students in both the lesson view and admin overview.
 *
 * @since  1.3.20
 */
class LessonModelWaitingListTest extends TestCase
{
     ...
     * tests can assert on them without needing a real database.
     *
