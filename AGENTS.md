...
Cloud Agents refresh dependencies via `.cursor/environment.json`, which runs:
...
That script installs PHP dependencies with Composer and installs the member SPA npm packages when they are not already present.
