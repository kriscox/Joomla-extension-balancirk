...
/**************************************************************************************************
 *                                                                                                 *
 *  Balancirk members VIEW                                                                         *
 *                                                                                                 *
 *    combines Joomla users with additional member data                                           *
 *                                                                                                 *
 **************************************************************************************************/
CREATE OR REPLACE VIEW `#__balancirk_members` AS
SELECT u.*,
    m.firstname,
    m.street,
    m.number,
...
CREATE TABLE IF NOT EXISTS `#__balancirk_parents` (
    ...
    UNIQUE (`parent`, `child`),
    CONSTRAINT `fk_parent_parent` FOREIGN KEY (parent) REFERENCES `#__balancirk_members_additional` (id),
    CONSTRAINT `fk_parent_childs` FOREIGN KEY (child) REFERENCES `#__balancirk_students` (id)
);
/**************************************************************************************************
 *                                                                                                 * 
...
CREATE TABLE IF NOT EXISTS `#__balancirk_lessons` (
    ...
    `subscription_email_subject` varchar(255) DEFAULT NULL,
    ...
    `ordering` int(11) NOT NULL DEFAULT 0,
    CONSTRAINT `fk_lesson_types` FOREIGN KEY (`type`) REFERENCES `#__balancirk_types` (id)
);
/**************************************************************************************************
 *                                                                                                 * 
...
CREATE TABLE IF NOT EXISTS `#__balancirk_subscriptions`(
    ...
    CONSTRAINT `fk_subscription_lesson` FOREIGN KEY (lesson) REFERENCES `#__balancirk_lessons` (id),
    CONSTRAINT `fk_subscription_student` FOREIGN KEY (student) REFERENCES `#__balancirk_students` (id)
);
/**************************************************************************************************
 *                                                                                                 *
 *  Balancirk subscriptions VIEW                                                                   *
 *                                                                                                 *
 *    denormalized subscription data for easy filtering and display                               *
 *                                                                                                 *
 **************************************************************************************************/
CREATE OR REPLACE VIEW `#__balancirk_subscriptions_view` AS
SELECT s.`id` as id,
    t.`id` as 'studentid',
    t.`name`,
    t.`firstname`,
    ...
    l.`start`,
    l.`end`,
    l.`start_registration`,
    l.`end_registration`,
    l.`state`,
    s.`subscribed`
FROM `#__balancirk_subscriptions` as s
    INNER JOIN `#__balancirk_lessons` as l ON s.`lesson` = l.`id`
    INNER JOIN `#__balancirk_students` as t on s.`student` = t.`id`;
/**************************************************************************************************
 *                                                                                                 *
 *  Balancirk lessons complete VIEW                                                                *
 *                                                                                                 *
 *    provides lesson overview with calculated student counts for frontend display                *
 *                                                                                                 *
 **************************************************************************************************/
...
/**************************************************************************************************
 *                                                                                                 *
 *  Balancirk presences VIEW                                                                       *
 *                                                                                                 *
 *    denormalized presence data with student and lesson names                                    *
 *                                                                                                 *
 **************************************************************************************************/
CREATE OR REPLACE VIEW `#__balancirk_presences_view` AS
SELECT p.`id`,
    s.`name`,
    s.`firstname`,
    l.`name` as `lesson`,
...
FROM `#__balancirk_presences` p
    INNER JOIN `#__balancirk_students` s ON p.`student` = s.`id`
    INNER JOIN `#__balancirk_lessons` l ON p.`lesson` = l.`id`;
/**************************************************************************************************
 *                                                                                                 * 
...
/**************************************************************************************************
 *                                                                                                 *
 *  Balancirk subscriptions accounting VIEW                                                       *
 *                                                                                                 *
 *    denormalized subscription data for comptable/accounting exports                             *
 *                                                                                                 *
 **************************************************************************************************/
CREATE OR REPLACE VIEW `#__balancirk_subscriptions_comptable` AS
SELECT m.`firstname` AS `firstname`,
    m.`name` AS `name`,
    concat(m.`street`, ' ', m.`number`) AS `address`,
    m.`bus` AS `bus`,
    ...
    st.`uitpas` AS `uitpas`,
    st.`mutuality` AS `mutuality`
FROM `#__balancirk_subscriptions_view` s
